function analizar_chebyshevII()
    % =========================================================================
    % analizar_chebyshevII - Análisis de escalabilidad para Chebyshev II
    % =========================================================================
    
    fprintf('\n%s\n', repmat('=', 1, 70));
    fprintf('  ANÁLISIS DE ESCALABILIDAD - CHEBYSHEV II\n');
    fprintf('%s\n', repmat('=', 1, 70));
    
    % -------------------------------------------------------------------------
    % CONFIGURACIÓN ESPECÍFICA PARA CHEBYSHEV II
    % Chebyshev II tiene características similares a Chebyshev I pero con
    ...
    % -------------------------------------------------------------------------
    % PARÁMETROS DE PRUEBA (constantes para todos los métodos)
    % -------------------------------------------------------------------------
    rs = 1;
    ...
    % -------------------------------------------------------------------------
    % VERIFICAR ÓRDENES REALES DE CHEBYSHEV II
    % -------------------------------------------------------------------------
    fprintf('\n--- VERIFICANDO ÓRDENES DE CHEBYSHEV II ---\n');
    ...
    for i = 1:size(especificaciones, 1)
        ...
        ordenes_reales(i) = nch2;
        
        fprintf('Caso %2d: amax=%.2f, amin=%.1f, wswp=%.2f → Orden Chebyshev II = %2d\n', ...
            i, amax, amin, wswp, nch2);
    end
    
    % -------------------------------------------------------------------------
    % MEDICIONES PARA CHEBYSHEV II
    % -------------------------------------------------------------------------
    resultados = struct();
    ...
    resultados.z = medir_chebyshevII_z(especificaciones, ordenes_reales, rs, impedancias);
    
    % 2. Transformación en Frecuencia (f)
    fprintf('\n--- TRANSFORMACIÓN EN FRECUENCIA (f) ---\n');
    resultados.f = medir_chebyshevII_f(especificaciones, ordenes_reales, rs, frecuencias);
    
    % 3. Frecuencia + Impedancia (fz)
    fprintf('\n--- FRECUENCIA + IMPEDANCIA (fz) ---\n');
    resultados.fz = medir_chebyshevII_fz(especificaciones, ordenes_reales, rs, frecuencias, impedancias);
    
    % 4. Pasaaltas (hp)
    fprintf('\n--- PASAALTAS (hp) ---\n');
    resultados.hp = medir_chebyshevII_hp(especificaciones, ordenes_reales, rs, frecuencias);
    
    % 5. Pasaaltas + Impedancia (hpz)
    fprintf('\n--- PASAALTAS + IMPEDANCIA (hpz) ---\n');
    resultados.hpz = medir_chebyshevII_hpz(especificaciones, ordenes_reales, rs, frecuencias, impedancias);
    
    % 6. Pasabanda (bp)
    fprintf('\n--- PASABANDA (bp) ---\n');
    resultados.bp = medir_chebyshevII_bp(especificaciones, ordenes_reales, rs, bandas);
    
    % 7. Pasabanda + Impedancia (bpz)
    fprintf('\n--- PASABANDA + IMPEDANCIA (bpz) ---\n');
    resultados.bpz = medir_chebyshevII_bpz(especificaciones, ordenes_reales, rs, bandas, impedancias);
    
    % 8. Banda Eliminada (bs)
    fprintf('\n--- BANDA ELIMINADA (bs) ---\n');
    resultados.bs = medir_chebyshevII_bs(especificaciones, ordenes_reales, rs, bandas);
    
    % 9. Banda Eliminada + Impedancia (bsz)
    fprintf('\n--- BANDA ELIMINADA + IMPEDANCIA (bsz) ---\n');
    resultados.bsz = medir_chebyshevII_bsz(especificaciones, ordenes_reales, rs, bandas, impedancias);
    
    % -------------------------------------------------------------------------
    % GUARDAR Y GRAFICAR
    % -------------------------------------------------------------------------
    guardar_resultados_chebyshevII(resultados);
    graficar_chebyshevII(resultados);
    analizar_complejidad_chebyshevII(resultados);
    
    fprintf('\n%s\n', repmat('=', 1, 70));
    fprintf('  ANÁLISIS DE CHEBYSHEV II COMPLETADO\n');
    fprintf('%s\n', repmat('=', 1, 70));
end

% =========================================================================
% FUNCIONES DE MEDICIÓN PARA CHEBYSHEV II
% =========================================================================

function data = medir_chebyshevII_z(especificaciones, ordenes, rs, impedancias)
    ...
    for i = 1:num_casos
        ...
        for z = 1:num_impedancias
            ...
            tiempos(i, z) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'z', 'cheby2', 'none', palp_z));
        end
...
function data = medir_chebyshevII_f(especificaciones, ordenes, rs, frecuencias)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            ...
            tiempos(i, f) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'f', 'cheby2', 'none', butlp_frec));
        end
...
function data = medir_chebyshevII_fz(especificaciones, ordenes, rs, frecuencias, impedancias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    num_impedancias = length(impedancias);
        ...
        for f = 1:num_frecuencias
            for z = 1:num_impedancias
                ...
                tiempos(i, f, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'fz', 'cheby2', 'none', [butlp_frec, palp_z]));
            end
        end
...
function data = medir_chebyshevII_hp(especificaciones, ordenes, rs, frecuencias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    tiempos = zeros(num_casos, num_frecuencias);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            ...
            tiempos(i, f) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'hp', 'cheby2', 'none', buthp_frec));
        end
...
function data = medir_chebyshevII_hpz(especificaciones, ordenes, rs, frecuencias, impedancias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    num_impedancias = length(impedancias);
    tiempos = zeros(num_casos, num_frecuencias, num_impedancias);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            for z = 1:num_impedancias
                ...
                tiempos(i, f, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'hpz', 'cheby2', 'none', [buthp_frec, palp_z]));
            end
        end
...
function data = medir_chebyshevII_bp(especificaciones, ordenes, rs, bandas)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            ...
            tiempos(i, b) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'bp', 'cheby2', 'none', [butbp_frec, butbp_bw]));
        end
...
function data = medir_chebyshevII_bpz(especificaciones, ordenes, rs, bandas, impedancias)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    num_impedancias = length(impedancias);
        ...
        for b = 1:num_bandas
            for z = 1:num_impedancias
                ...
                tiempos(i, b, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'bpz', 'cheby2', 'none', [butbp_frec, butbp_bw, palp_z]));
            end
        end
...
function data = medir_chebyshevII_bs(especificaciones, ordenes, rs, bandas)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    tiempos = zeros(num_casos, num_bandas);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            ...
            tiempos(i, b) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'bs', 'cheby2', 'none', [butbs_frec, butbs_bw]));
        end
...
function data = medir_chebyshevII_bsz(especificaciones, ordenes, rs, bandas, impedancias)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    num_impedancias = length(impedancias);
    tiempos = zeros(num_casos, num_bandas, num_impedancias);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            for z = 1:num_impedancias
                ...
                tiempos(i, b, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'bsz', 'cheby2', 'none', [butbs_frec, butbs_bw, palp_z]));
            end
        end
...
% =========================================================================
% FUNCIONES DE UTILIDAD PARA CHEBYSHEV II
% =========================================================================

function guardar_resultados_chebyshevII(resultados)

    mkdir('resultados_chebyshevII');
    mkdir('resultados_chebyshevII/graficas');
    mkdir('resultados_chebyshevII/datos');

    timestamp = datestr(now, 'yyyymmdd_HHMMSS');

    archivo = sprintf('resultados_chebyshevII/datos/chebyshevII_%s.mat', timestamp);

    save(archivo, 'resultados', '-v7.3');
    save('resultados_chebyshevII/ultimo_analisis.mat', 'resultados', '-v7.3');

    fprintf('  Resultados guardados en: %s\n', archivo);
end

function graficar_chebyshevII(resultados)
    figure('Position', [100, 100, 1200, 800]);
    
    subplot(2,3,1);
    tiempos_medios = mean(resultados.z.tiempos, 2);
    plot(resultados.z.ordenes, tiempos_medios, 'bo-', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('Chebyshev II order');
    ylabel('Time (s)');
    title('Chebyshev II - Impedance (z-LP)');
    grid on;
    ...
    xlabel('Chebyshev II order');
    ylabel('Time (s)');
    title('Chebyshev II - Frequency (f-LP)');
    grid on;
    ...
    xlabel('Chebyshev II order');
    ylabel('Time (s)');
    title('Chebyshev II - High-pass (HP)');
    grid on;
    ...
    xlabel('Chebyshev II order');
    ylabel('Time (s)');
    title('Chebyshev II - Band-pass (BP)');
    grid on;
    ...
    xlabel('Chebyshev II order');
    ylabel('Time (s)');
    title('Chebyshev II - Band reject (BR)');
    grid on;
    ...
    xlabel('Chebyshev II order');
    ylabel('Time (s)');
    title('Chebyshev II - Double Transformations');
    legend('fz', 'hpz', 'bpz', 'bsz', 'Location', 'best');
    grid on;
    
    sgtitle('Scalability analysis - Chebyshev II');
    saveas(gcf, 'resultados_chebyshevII/graficas/chebyshevII_completo.png');
end

function analizar_complejidad_chebyshevII(resultados)
    % =========================================================================
    % analizar_complejidad_chebyshevII - Analiza complejidad de TODAS las
    % transformaciones para Chebyshev II
    % =========================================================================
    
    fid = fopen('resultados_chebyshevII/analisis_complejidad.txt', 'w');
    fprintf(fid, 'ANÁLISIS DE COMPLEJIDAD - CHEBYSHEV II\n');
    fprintf(fid, '======================================\n\n');
    fprintf(fid, 'Fecha: %s\n\n', datestr(now));
    ...
    fprintf('  Resultados guardados en: resultados_chebyshevII/analisis_complejidad.txt\n');
end