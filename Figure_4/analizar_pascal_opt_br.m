function analizar_pascal_opt_br()
    % =========================================================================
    % analizar_pascal_opt_br - Análisis de escalabilidad para Pascal OPT_BR
    % =========================================================================
    
    fprintf('\n%s\n', repmat('=', 1, 70));
    fprintf('  ANÁLISIS DE ESCALABILIDAD - PASCAL OPT_BR\n');
    fprintf('%s\n', repmat('=', 1, 70));
    
    % -------------------------------------------------------------------------
    % CONFIGURACIÓN ESPECÍFICA PARA PASCAL OPT_BR
    ...
    % -------------------------------------------------------------------------
    % VERIFICAR ÓRDENES REALES DE PASCAL OPT_BR
    % -------------------------------------------------------------------------
    fprintf('\n--- VERIFICANDO ÓRDENES DE PASCAL OPT_BR ---\n');
    ...
    % -------------------------------------------------------------------------
    % MEDICIONES PARA PASCAL OPT_BR
    % (MISMA ESTRUCTURA QUE OPT_BP PERO CON TIPO 'opt_br')
    % -------------------------------------------------------------------------
    resultados = struct();
    ...
    % 1. Caso Ideal
    fprintf('\n--- CASO IDEAL ---\n');
    resultados.ideal = medir_pascal_opt_br_ideal(especificaciones, ordenes_pascal, rs);
    
    % 2. Transformación de Impedancia (z)
    fprintf('\n--- TRANSFORMACIÓN DE IMPEDANCIA (z) ---\n');
    resultados.z = medir_pascal_opt_br_z(especificaciones, ordenes_pascal, rs, impedancias);
    
    % 3. Transformación en Frecuencia (f)
    fprintf('\n--- TRANSFORMACIÓN EN FRECUENCIA (f) ---\n');
    resultados.f = medir_pascal_opt_br_f(especificaciones, ordenes_pascal, rs, frecuencias);
    
    % 4. Frecuencia + Impedancia (fz)
    fprintf('\n--- FRECUENCIA + IMPEDANCIA (fz) ---\n');
    resultados.fz = medir_pascal_opt_br_fz(especificaciones, ordenes_pascal, rs, frecuencias, impedancias);
    
    % 5. Pasaaltas (hp)
    fprintf('\n--- PASAALTAS (hp) ---\n');
    resultados.hp = medir_pascal_opt_br_hp(especificaciones, ordenes_pascal, rs, frecuencias_hp);
    
    % 6. Pasaaltas + Impedancia (hpz)
    fprintf('\n--- PASAALTAS + IMPEDANCIA (hpz) ---\n');
    resultados.hpz = medir_pascal_opt_br_hpz(especificaciones, ordenes_pascal, rs, frecuencias_hp, impedancias);
    
    % 7. Pasabanda (bp)
    fprintf('\n--- PASABANDA (bp) ---\n');
    resultados.bp = medir_pascal_opt_br_bp(especificaciones, ordenes_pascal, rs, bandas);
    
    % 8. Pasabanda + Impedancia (bpz)
    fprintf('\n--- PASABANDA + IMPEDANCIA (bpz) ---\n');
    resultados.bpz = medir_pascal_opt_br_bpz(especificaciones, ordenes_pascal, rs, bandas, impedancias);
    
    % 9. Banda Eliminada (bs)
    fprintf('\n--- BANDA ELIMINADA (bs) ---\n');
    resultados.bs = medir_pascal_opt_br_bs(especificaciones, ordenes_pascal, rs, bandas);
    
    % 10. Banda Eliminada + Impedancia (bsz)
    fprintf('\n--- BANDA ELIMINADA + IMPEDANCIA (bsz) ---\n');
    resultados.bsz = medir_pascal_opt_br_bsz(especificaciones, ordenes_pascal, rs, bandas, impedancias);
    
    % -------------------------------------------------------------------------
    % GUARDAR Y GRAFICAR
    % -------------------------------------------------------------------------
    guardar_resultados_pascal_opt_br(resultados);
    graficar_pascal_opt_br(resultados);
    analizar_complejidad_pascal_opt_br(resultados);
    
    fprintf('\n%s\n', repmat('=', 1, 70));
    fprintf('  ANÁLISIS DE PASCAL OPT_BR COMPLETADO\n');
    fprintf('%s\n', repmat('=', 1, 70));
end

% =========================================================================
% FUNCIONES DE MEDICIÓN PARA PASCAL OPT_BR
% =========================================================================

function data = medir_pascal_opt_br_ideal(especificaciones, ordenes, rs)
    ...
    for i = 1:num_casos
        ...
        tiempos(i) = timeit(@() calcular_filtros(...
            amax, amin, wswp, rs, 'none', 'opt_br', 'real', []));
...
function data = medir_pascal_opt_br_z(especificaciones, ordenes, rs, impedancias)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for z = 1:num_impedancias
            ...
            tiempos(i, z) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'z', 'opt_br', 'real', palp_z));
        end
...
function data = medir_pascal_opt_br_f(especificaciones, ordenes, rs, frecuencias)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            ...
            tiempos(i, f) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'f', 'opt_br', 'real', palp_frec));
        end
...
function data = medir_pascal_opt_br_fz(especificaciones, ordenes, rs, frecuencias, impedancias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    num_impedancias = length(impedancias);
        ...
        for f = 1:num_frecuencias
            for z = 1:num_impedancias
                ...
                tiempos(i, f, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'fz', 'opt_br', 'real', [palp_frec, palp_z]));
            end
        end
...
function data = medir_pascal_opt_br_hp(especificaciones, ordenes, rs, frecuencias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    tiempos = zeros(num_casos, num_frecuencias);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            ...
            tiempos(i, f) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'hp', 'opt_br', 'real', pahp_frec));
        end
...
function data = medir_pascal_opt_br_hpz(especificaciones, ordenes, rs, frecuencias, impedancias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    num_impedancias = length(impedancias);
    tiempos = zeros(num_casos, num_frecuencias, num_impedancias);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            for z = 1:num_impedancias
                ...
                tiempos(i, f, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'hpz', 'opt_br', 'real', [pahp_frec, palp_z]));
            end
        end
...
function data = medir_pascal_opt_br_bp(especificaciones, ordenes, rs, bandas)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            ...
            tiempos(i, b) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'bp', 'opt_br', 'real', [pabp_frec, pabp_bw]));
        end
...
function data = medir_pascal_opt_br_bpz(especificaciones, ordenes, rs, bandas, impedancias)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    num_impedancias = length(impedancias);
        ...
        for b = 1:num_bandas
            for z = 1:num_impedancias
                ...
                tiempos(i, b, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'bpz', 'opt_br', 'real', [pabp_frec, pabp_bw, palp_z]));
            end
        end
...
function data = medir_pascal_opt_br_bs(especificaciones, ordenes, rs, bandas)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    tiempos = zeros(num_casos, num_bandas);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            ...
            tiempos(i, b) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'bs', 'opt_br', 'real', [pabs_frec, pabs_bw]));
        end
...
function data = medir_pascal_opt_br_bsz(especificaciones, ordenes, rs, bandas, impedancias)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    num_impedancias = length(impedancias);
    tiempos = zeros(num_casos, num_bandas, num_impedancias);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            for z = 1:num_impedancias
                ...
                tiempos(i, b, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'bsz', 'opt_br', 'real', [pabs_frec, pabs_bw, palp_z]));
            end
        end
...
% =========================================================================
% FUNCIONES DE UTILIDAD PARA PASCAL OPT_BR
% =========================================================================

function guardar_resultados_pascal_opt_br(resultados)

    mkdir('resultados_pascal_opt_br');
    mkdir('resultados_pascal_opt_br/graficas');
    mkdir('resultados_pascal_opt_br/datos');

    timestamp = datestr(now, 'yyyymmdd_HHMMSS');

    archivo = sprintf('resultados_pascal_opt_br/datos/pascal_opt_br_%s.mat', timestamp);

    save(archivo, 'resultados', '-v7.3');
    save('resultados_pascal_opt_br/ultimo_analisis.mat', 'resultados', '-v7.3');

    fprintf('  Resultados guardados en: %s\n', archivo);
end

function graficar_pascal_opt_br(resultados)
    ...
    xlabel('Pascal order');
    ylabel('Time (s)');
    title('Pascal OPT\_BR - Impedance (z-LP)');
    grid on;
    ...
    xlabel('Pascal order');
    ylabel('Time (s)');
    title('Pascal OPT\_BR - Frequency (f-LP)');
    grid on;
    ...
    xlabel('Pascal order');
    ylabel('Time (s)');
    title('Pascal OPT\_BR - High-pass (HP)');
    grid on;
    ...
    xlabel('Pascal order');
    ylabel('Time (s)');
    title('Pascal OPT\_BR - Band-pass (BP)');
    grid on;
    
    % 5. Banda eliminada (bs)  <-- AQUÍ está el cambio clave
    subplot(2,3,5);
    tiempos_medios = mean(resultados.bs.tiempos, 2);
    plot(resultados.bs.ordenes, tiempos_medios, 'co-', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('Pascal order');
    ylabel('Time (s)');
    title('Pascal OPT\_BR - Band reject (BR)');
    grid on;
    
    % 6. Dobles transformaciones
    subplot(2,3,6);
    ...
    xlabel('Orden Pascal');
    ylabel('Tiempo (s)');
    title('Pascal OPT\_BR - Double Transformations');
    legend('fz', 'hpz', 'bpz', 'bsz', 'Location', 'best');
    grid on;
    
    sgtitle('Scalability analysis - Pascal OPT\_BR');
    saveas(gcf, 'resultados_pascal_opt_br/graficas/pascal_opt_br_completo.png');
end

function analizar_complejidad_pascal_opt_br(resultados)
    % =========================================================================
    % analizar_complejidad_pascal_opt_br - Analiza complejidad de TODAS las
    % transformaciones para Pascal OPT_BR
    % =========================================================================
    
    fid = fopen('resultados_pascal_opt_br/analisis_complejidad.txt', 'w');
    fprintf(fid, 'ANÁLISIS DE COMPLEJIDAD - PASCAL OPT_BR\n');
    fprintf(fid, '======================================\n\n');
    fprintf(fid, 'Fecha: %s\n\n', datestr(now));
    ...
    fprintf('  Resultados guardados en: resultados_pascal_opt_br/analisis_complejidad.txt\n');
end