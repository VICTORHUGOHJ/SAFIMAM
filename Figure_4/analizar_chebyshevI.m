function analizar_chebyshevI()
    % =========================================================================
    % analizar_chebyshevI - Análisis de escalabilidad para Chebyshev I
    % =========================================================================
    
    fprintf('\n%s\n', repmat('=', 1, 70));
    fprintf('  ANÁLISIS DE ESCALABILIDAD - CHEBYSHEV I\n');
    fprintf('%s\n', repmat('=', 1, 70));
    
    % -------------------------------------------------------------------------
    % CONFIGURACIÓN ESPECÍFICA PARA CHEBYSHEV I
    % (Chebyshev I typically requires lower order than Butterworth)
    % -------------------------------------------------------------------------
    ...
    % -------------------------------------------------------------------------
    % VERIFICAR ÓRDENES REALES DE CHEBYSHEV I
    % -------------------------------------------------------------------------
    fprintf('\n--- VERIFICANDO ÓRDENES DE CHEBYSHEV I ---\n');
    ...
    for i = 1:size(especificaciones, 1)
        ...
        ordenes_reales(i) = nch1;
        
        fprintf('Caso %2d: amax=%.2f, amin=%.1f, wswp=%.2f → Orden Chebyshev I = %2d\n', ...
            i, amax, amin, wswp, nch1);
    end
    
    % -------------------------------------------------------------------------
    % MEDICIONES PARA CHEBYSHEV I
    % (Similar a Butterworth pero cambiando tipo_filtro a 'cheby1')
    % -------------------------------------------------------------------------
    resultados = struct();
    ...
    resultados.z = medir_chebyshevI_z(especificaciones, ordenes_reales, rs, impedancias);
    
    % 2. Transformación en Frecuencia (f)
    fprintf('\n--- TRANSFORMACIÓN EN FRECUENCIA (f) ---\n');
    resultados.f = medir_chebyshevI_f(especificaciones, ordenes_reales, rs, frecuencias);
    
    % 3. Frecuencia + Impedancia (fz)
    fprintf('\n--- FRECUENCIA + IMPEDANCIA (fz) ---\n');
    resultados.fz = medir_chebyshevI_fz(especificaciones, ordenes_reales, rs, frecuencias, impedancias);
    
    % 4. Pasaaltas (hp)
    fprintf('\n--- PASAALTAS (hp) ---\n');
    resultados.hp = medir_chebyshevI_hp(especificaciones, ordenes_reales, rs, frecuencias);
    
    % 5. Pasaaltas + Impedancia (hpz)
    fprintf('\n--- PASAALTAS + IMPEDANCIA (hpz) ---\n');
    resultados.hpz = medir_chebyshevI_hpz(especificaciones, ordenes_reales, rs, frecuencias, impedancias);
    
    % 6. Pasabanda (bp)
    fprintf('\n--- PASABANDA (bp) ---\n');
    resultados.bp = medir_chebyshevI_bp(especificaciones, ordenes_reales, rs, bandas);
    
    % 7. Pasabanda + Impedancia (bpz)
    fprintf('\n--- PASABANDA + IMPEDANCIA (bpz) ---\n');
    resultados.bpz = medir_chebyshevI_bpz(especificaciones, ordenes_reales, rs, bandas, impedancias);
    
    % 8. Banda Eliminada (bs)
    fprintf('\n--- BANDA ELIMINADA (bs) ---\n');
    resultados.bs = medir_chebyshevI_bs(especificaciones, ordenes_reales, rs, bandas);
    
    % 9. Banda Eliminada + Impedancia (bsz)
    fprintf('\n--- BANDA ELIMINADA + IMPEDANCIA (bsz) ---\n');
    resultados.bsz = medir_chebyshevI_bsz(especificaciones, ordenes_reales, rs, bandas, impedancias);
    
    % -------------------------------------------------------------------------
    % GUARDAR Y GRAFICAR
    % -------------------------------------------------------------------------
    guardar_resultados_chebyshevI(resultados);
    graficar_chebyshevI(resultados);
    analizar_complejidad_chebyshevI(resultados);
    
    fprintf('\n%s\n', repmat('=', 1, 70));
    fprintf('  ANÁLISIS DE CHEBYSHEV I COMPLETADO\n');
    fprintf('%s\n', repmat('=', 1, 70));
end

% =========================================================================
% FUNCIONES DE MEDICIÓN PARA CHEBYSHEV I
% (Cambian tipo_filtro a 'cheby1')
% =========================================================================

function data = medir_chebyshevI_z(especificaciones, ordenes, rs, impedancias)
    ...
    for i = 1:num_casos
        ...
        for z = 1:num_impedancias
            ...
            tiempos(i, z) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'z', 'cheby1', 'none', palp_z));
        end
...
function data = medir_chebyshevI_f(especificaciones, ordenes, rs, frecuencias)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            ...
            tiempos(i, f) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'f', 'cheby1', 'none', butlp_frec));
        end
...
function data = medir_chebyshevI_fz(especificaciones, ordenes, rs, frecuencias, impedancias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    num_impedancias = length(impedancias);
        ...
        for f = 1:num_frecuencias
            for z = 1:num_impedancias
                ...
                tiempos(i, f, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'fz', 'cheby1', 'none', [butlp_frec, palp_z]));
            end
        end
...
function data = medir_chebyshevI_hp(especificaciones, ordenes, rs, frecuencias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    tiempos = zeros(num_casos, num_frecuencias);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            ...
            tiempos(i, f) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'hp', 'cheby1', 'none', buthp_frec));
        end
...
function data = medir_chebyshevI_hpz(especificaciones, ordenes, rs, frecuencias, impedancias)
    num_casos = size(especificaciones, 1);
    num_frecuencias = length(frecuencias);
    num_impedancias = length(impedancias);
    tiempos = zeros(num_casos, num_frecuencias, num_impedancias);
    ...
    for i = 1:num_casos
        ...
        for f = 1:num_frecuencias
            for z = 1:num_impedancias
                ...
                tiempos(i, f, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'hpz', 'cheby1', 'none', [buthp_frec, palp_z]));
            end
        end
...
function data = medir_chebyshevI_bp(especificaciones, ordenes, rs, bandas)
    num_casos = size(especificaciones, 1);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            ...
            tiempos(i, b) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'bp', 'cheby1', 'none', [butbp_frec, butbp_bw]));
        end
...
function data = medir_chebyshevI_bpz(especificaciones, ordenes, rs, bandas, impedancias)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    num_impedancias = length(impedancias);
        ...
        for b = 1:num_bandas
            for z = 1:num_impedancias
                ...
                tiempos(i, b, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'bpz', 'cheby1', 'none', [butbp_frec, butbp_bw, palp_z]));
            end
        end
...
function data = medir_chebyshevI_bs(especificaciones, ordenes, rs, bandas)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    tiempos = zeros(num_casos, num_bandas);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            ...
            tiempos(i, b) = timeit(@() calcular_filtros(...
                amax, amin, wswp, rs, 'bs', 'cheby1', 'none', [butbs_frec, butbs_bw]));
        end
...
function data = medir_chebyshevI_bsz(especificaciones, ordenes, rs, bandas, impedancias)
    num_casos = size(especificaciones, 1);
    num_bandas = size(bandas, 1);
    num_impedancias = length(impedancias);
    tiempos = zeros(num_casos, num_bandas, num_impedancias);
    ...
    for i = 1:num_casos
        ...
        for b = 1:num_bandas
            for z = 1:num_impedancias
                ...
                tiempos(i, b, z) = timeit(@() calcular_filtros(...
                    amax, amin, wswp, rs, 'bsz', 'cheby1', 'none', [butbs_frec, butbs_bw, palp_z]));
            end
        end
...
% =========================================================================
% FUNCIONES DE UTILIDAD PARA CHEBYSHEV I
% =========================================================================

function guardar_resultados_chebyshevI(resultados)

    mkdir('resultados_chebyshevI');
    mkdir('resultados_chebyshevI/graficas');
    mkdir('resultados_chebyshevI/datos');

    timestamp = datestr(now, 'yyyymmdd_HHMMSS');

    archivo = sprintf('resultados_chebyshevI/datos/chebyshevI_%s.mat', timestamp);

    save(archivo, 'resultados', '-v7.3');
    save('resultados_chebyshevI/ultimo_analisis.mat', 'resultados', '-v7.3');

    fprintf('  Resultados guardados en: %s\n', archivo);
end

function graficar_chebyshevI(resultados)
    % Similar a graficar_butterworth pero con títulos diferentes
    ...
    ylabel('Tiempo (s)');
    title('Chebyshev I - Impedancia (z)');
    grid on;
    ...
    xlabel('Orden del filtro');
    ylabel('Tiempo (s)');
    title('Chebyshev I - Frecuencia (f)');
    grid on;
    ...
    xlabel('Orden del filtro');
    ylabel('Tiempo (s)');
    title('Chebyshev I - Pasaaltas (hp)');
    grid on;
    ...
    xlabel('Orden del filtro');
    ylabel('Tiempo (s)');
    title('Chebyshev I - Pasabanda (bp)');
    grid on;
    ...
    xlabel('Orden del filtro');
    ylabel('Tiempo (s)');
    title('Chebyshev I - Banda Eliminada (bs)');
    grid on;
    ...
    xlabel('Orden del filtro');
    ylabel('Tiempo (s)');
    title('Chebyshev I - Dobles Transformaciones');
    legend('fz', 'hpz', 'bpz', 'bsz', 'Location', 'best');
    grid on;
    
    sgtitle('Análisis de Escalabilidad - Chebyshev I');
    saveas(gcf, 'resultados_chebyshevI/graficas/chebyshevI_completo.png');
end

function analizar_complejidad_chebyshevI(resultados)
    % =========================================================================
    % analizar_complejidad_chebyshevI - Analiza complejidad de TODAS las
    % transformaciones para Chebyshev I
    % =========================================================================
    
    fid = fopen('resultados_chebyshevI/analisis_complejidad.txt', 'w');
    fprintf(fid, 'ANÁLISIS DE COMPLEJIDAD - CHEBYSHEV I\n');
    fprintf(fid, '=====================================\n\n');
    fprintf(fid, 'Fecha: %s\n\n', datestr(now));
    ...
    fprintf('  Resultados guardados en: resultados_chebyshevI/analisis_complejidad.txt\n');
end