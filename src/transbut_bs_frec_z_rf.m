function varargout = transbut_bs_frec_z_rf(varargin)
% TRANSBUT_BS_FREC_Z_RF MATLAB code for transbut_bs_frec_z_rf.fig
%      TRANSBUT_BS_FREC_Z_RF, by itself, creates a new TRANSBUT_BS_FREC_Z_RF or raises the existing
%      singleton*.
%
%      H = TRANSBUT_BS_FREC_Z_RF returns the handle to a new TRANSBUT_BS_FREC_Z_RF or the handle to
%      the existing singleton*.
%
%      TRANSBUT_BS_FREC_Z_RF('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRANSBUT_BS_FREC_Z_RF.M with the given input arguments.
%
%      TRANSBUT_BS_FREC_Z_RF('Property','Value',...) creates a new TRANSBUT_BS_FREC_Z_RF or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before transbut_bs_frec_z_rf_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to transbut_bs_frec_z_rf_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help transbut_bs_frec_z_rf

% Last Modified by GUIDE v2.5 02-Oct-2022 20:37:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @transbut_bs_frec_z_rf_OpeningFcn, ...
                   'gui_OutputFcn',  @transbut_bs_frec_z_rf_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before transbut_bs_frec_z_rf is made visible.
function transbut_bs_frec_z_rf_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to transbut_bs_frec_z_rf (see VARARGIN)
...
% Choose default command line output for transbut_bs_frec_z_rf
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes transbut_bs_frec_z_rf wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = transbut_bs_frec_z_rf_OutputFcn(hObject, eventdata, handles) 
...
global sysbut_bs_f sysch1_bs_f syse_bs_f sysch2_bs_f f_but f_cheby1 f_cheby2 f_elip 
if f_but == 1 && f_cheby1==0 && f_cheby2==0 && f_elip==0
h=bodeoptions; 
h.FreqUnits='Hz';
h.PhaseVisible='off';
bodeplot(sysbut_bs_f,h,'c')
set(findall(gcf,'type','line'),'linewidth',1.3)
grid on;
axes(handles.rfbutbs_frec_z);
legend('Butterworth BS')

elseif f_but == 0 && f_cheby1==1 && f_cheby2==0 && f_elip==0
h=bodeoptions; 
h.FreqUnits='Hz';
h.PhaseVisible='off';
bodeplot(sysch1_bs_f,h,'red')
set(findall(gcf,'type','line'),'linewidth',1.3)
grid on;
axes(handles.rfbutbs_frec_z);
legend('Chebyshev I BS')

elseif f_but == 0 && f_cheby1==0 && f_cheby2==1 && f_elip==0
h=bodeoptions; 
h.FreqUnits='Hz';
h.PhaseVisible='off';
bodeplot(sysch2_bs_f,h,'k')
set(findall(gcf,'type','line'),'linewidth',1.3)
grid on;
axes(handles.rfbutbs_frec_z);
legend('Chebyshev II BS')

else
h=bodeoptions; 
h.FreqUnits='Hz';
h.PhaseVisible='off';
bodeplot(syse_bs_f,h,'magenta')
set(findall(gcf,'type','line'),'linewidth',1.3)
grid on;
axes(handles.rfbutbs_frec_z);
legend('Elliptic BS')
...
% --- Executes on button press in fin31.
function fin31_Callback(hObject, eventdata, handles)
% hObject    handle to fin31 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
clear,clc,close all

% --- Executes on button press in redbut_bs_frec_z.
function redbut_bs_frec_z_Callback(hObject, eventdata, handles)
% hObject    handle to redbut_bs_frec_z (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global kb zb pb numb denb rs RL Lbu_lp_t Cbu_lp_t sysbut_lp Lbu_lp_pi Cbu_lp_pi nb nch1 f_but f_cheby1 zc1 pc1 kc1 numch1 dench1 sysch1_lp Lch1_lp_pi Cch1_lp_pi Lch1_lp_t Cch1_lp_t f_cheby2 f_elip nch2 ne zc2 pc2 kc2 numch2 dench2 sysch2_lp ze pe ke nume dene syse_lp Lch2_lp_pi Cch2_lp_pi Le_lp_pi Ce_lp_pi Lch2_lp_t Cch2_lp_t Le_lp_t Ce_lp_t sysbut_bs_f sysch1_bs_f sysch2_bs_f syse_bs_f Wc_bs Bw_bs Lbu_serie_bs_t_f Cbu_serie_bs_t_f Lbu_paralelo_bs_t_f Cbu_paralelo_bs_t_f Lch1_serie_bs_t_f Cch1_serie_bs_t_f Lch1_paralelo_bs_t_f Cch1_paralelo_bs_t_f Lch2_serie_bs_t_f Cch2_serie_bs_t_f Lch2_paralelo_bs_t_f Cch2_paralelo_bs_t_f Le_serie_bs_t_f Ce_serie_bs_t_f Le_paralelo_bs_t_f Ce_paralelo_bs_t_f Lbu_serie_bs_pi_f Cbu_serie_bs_pi_f Lbu_paralelo_bs_pi_f Cbu_paralelo_bs_pi_f Lch1_serie_bs_pi_f Cch1_serie_bs_pi_f Lch1_paralelo_bs_pi_f Cch1_paralelo_bs_pi_f Lch2_serie_bs_pi_f Cch2_serie_bs_pi_f Lch2_paralelo_bs_pi_f Cch2_paralelo_bs_pi_f Le_serie_bs_pi_f Ce_serie_bs_pi_f Le_paralelo_bs_pi_f Ce_paralelo_bs_pi_f butbs_frec_z
set(gcf,'visible','off');
transbut_bs_frec_z_red(kb,zb,pb,numb,denb,rs,RL,Lbu_lp_t,Cbu_lp_t,sysbut_lp,Lbu_lp_pi,Cbu_lp_pi,nb,nch1,f_but,f_cheby1,zc1,pc1,kc1,numch1,dench1,sysch1_lp,Lch1_lp_pi,Cch1_lp_pi,Lch1_lp_t,Cch1_lp_t,f_cheby2,f_elip,nch2,ne,zc2,pc2,kc2,numch2,dench2,sysch2_lp,ze,pe,ke,nume,dene,syse_lp,Lch2_lp_pi,Cch2_lp_pi,Le_lp_pi,Ce_lp_pi,Lch2_lp_t,Cch2_lp_t,Le_lp_t,Ce_lp_t,sysbut_bs_f,sysch1_bs_f,sysch2_bs_f,syse_bs_f,Wc_bs,Bw_bs,Lbu_serie_bs_t_f,Cbu_serie_bs_t_f,Lbu_paralelo_bs_t_f,Cbu_paralelo_bs_t_f,Lch1_serie_bs_t_f,Cch1_serie_bs_t_f,Lch1_paralelo_bs_t_f,Cch1_paralelo_bs_t_f,Lch2_serie_bs_t_f,Cch2_serie_bs_t_f,Lch2_paralelo_bs_t_f,Cch2_paralelo_bs_t_f,Le_serie_bs_t_f,Ce_serie_bs_t_f,Le_paralelo_bs_t_f,Ce_paralelo_bs_t_f,Lbu_serie_bs_pi_f,Cbu_serie_bs_pi_f,Lbu_paralelo_bs_pi_f,Cbu_paralelo_bs_pi_f,Lch1_serie_bs_pi_f,Cch1_serie_bs_pi_f,Lch1_paralelo_bs_pi_f,Cch1_paralelo_bs_pi_f,Lch2_serie_bs_pi_f,Cch2_serie_bs_pi_f,Lch2_paralelo_bs_pi_f,Cch2_paralelo_bs_pi_f,Le_serie_bs_pi_f,Ce_serie_bs_pi_f,Le_paralelo_bs_pi_f,Ce_paralelo_bs_pi_f,butbs_frec_z)

% --- Executes on button press in reg19.
function reg19_Callback(hObject, eventdata, handles)
% hObject    handle to reg19 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global kb zb pb numb denb rs RL Lbu_lp_t Cbu_lp_t sysbut_lp Lbu_lp_pi Cbu_lp_pi nb nch1 f_but f_cheby1 zc1 pc1 kc1 numch1 dench1 sysch1_lp Lch1_lp_pi Cch1_lp_pi Lch1_lp_t Cch1_lp_t f_cheby2 f_elip nch2 ne zc2 pc2 kc2 numch2 dench2 sysch2_lp ze pe ke nume dene syse_lp Lch2_lp_pi Cch2_lp_pi Le_lp_pi Ce_lp_pi Lch2_lp_t Cch2_lp_t Le_lp_t Ce_lp_t
set(gcf,'visible','off');
