...
global np wc ws wsp amax amin nch1 bandera_bp bandera_br
np=varargin{1};
wc=varargin{2};
ws=varargin{3};
wsp=varargin{4};
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global sysprs syspminrs npa denparcial numerador denminparcial numin nump_max denp_max nump_min denp_min valor_rs bandera_bp bandera_br%p1 p2
valor_rs=handles.valor_rs;
if valor_rs<=0 || ~isnumeric(valor_rs) || ~isreal(valor_rs) || isnan(valor_rs)
warndlg('Enter the actual numeric Rs value','Warning');
else
if bandera_bp == 0 && bandera_br == 0 %p1==0 && p2==0
