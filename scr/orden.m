...
warndlg('Ingrese los datos necesarios o la relación de transición ingresada es menor a la unidad','Advertencia');
else
ws=wswp/10;
wp=1/10;
wc=wp*10;
...
if ~isnumeric(amax) || amax<=0 || ~isreal(amax) || ~isnumeric(amin) || amin<=0 || ~isreal(amin) || ~isnumeric(wswp) || wswp<=1 || wswp>=10 || ~isreal(wswp) || nb<=0 || isnan(nb) || isinf(nb) || ~isnumeric(nb) || ~isreal(nb) || nch1<=0 || isnan(nch1) || isinf(nch1) || ~isnumeric(nch1) || ~isreal(nch1) || nch2<=0 || isnan(nch2) || isinf(nch2) || ~isnumeric(nch2) || ~isreal(nch2) || ne<=0 || isnan(ne) || isinf(ne) || ~isnumeric(ne) || ~isreal(ne)
     warndlg('Ingresar especificaciones de diseño','Advertencia');
else
%%%%%%%%%%%%%%Filtro seleccionado
...
if nb<=1 || isnan(nb) || isinf(nb) || ~isnumeric(nb) || ~isreal(nb)
    warndlg('Función de Transferencia Indeterminada o Síntesis de Red No Disponible, se recomienda ingresar otras especificaciones de diseño','Advertencia');
...
if np<=1 || isnan(np) || isinf(np) || ~isnumeric(np) || ~isreal(np) || np>=22
    warndlg('Función de Transferencia Indeterminada o Síntesis de Red No Disponible, se recomienda ingresar otras especificaciones de diseño o colocar un orden de filtro menor a 21','Advertencia');
else
    orden('visible','off');
    filtropa_ft_realideal(np,wc,ws,wsp,amax,amin,nch1);
end
else
    warndlg('Seleccionar un método de aproximación','Advertencia');
end
end
...
wswp=handles.wswp;
if ~isnumeric(amax) || amax<=0 || ~isreal(amax) || ~isnumeric(amin) || amin<=0 || ~isreal(amin) || ~isnumeric(wswp) || wswp<=1 || wswp>=10 || ~isreal(wswp) || isnan(wswp)
warndlg('Función de Transferencia Indeterminada, se recomienda ingresar otras especificaciones de diseño o colocar un orden de filtro menor a 21','Advertencia');
else
