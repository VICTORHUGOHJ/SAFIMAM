...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[indpa,capa,capa_pi,indpa_pi]=pascalelementos(numerador,denparcial,1,valor_rs,npa)
[indpabr,capabr,capa_pibr,indpa_pibr]=pascalelementos(numin,denminparcial,1,valor_rs,npa)

Lpabp_lp_t=round(indpa,6);
Cpabp_lp_t=round(capa,6);
Lpabr_lp_t=round(indpabr,6);
Cpabr_lp_t=round(capabr,6);
Lpabp_lp_pi=round(indpa_pi,6);
Cpabp_lp_pi=round(capa_pi,6);
Lpabr_lp_pi=round(indpa_pibr,6);
Cpabr_lp_pi=round(capa_pibr,6);
...
set(handles.tipordpa,'String','Red tipo T');
...
if tawosnum_max~=taparealnum_max || tawosden_max~=taparealden_max
    warndlg('Síntesis de Red No Realizable','Advertencia');
    j=0;
else
%Resistores
erres=string({'Rs= ','RL= '});
...
set(handles.tipordpa,'String','Red tipo π');
f_paobp=get(handles.fpa_opt_bp_real,'Value');
f_paobr=get(handles.fpa_opt_br_real,'Value');
% [indpa,capa,capa_pi,indpa_pi]=pascalelementos(numerador,denparcial,1,valor_rs,npa);
% [indpabr,capabr,capa_pibr,indpa_pibr]=pascalelementos(numin,denminparcial,1,valor_rs,npa);
...
elseif tawosnum_max~=taparealnum_max || tawosden_max~=taparealden_max
    warndlg('Síntesis de Red No Realizable','Advertencia');
