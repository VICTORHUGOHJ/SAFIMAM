...
global sysprs syspminrs npa denparcial numerador denminparcial numin nump_max denp_max nump_min denp_min valor_rs Lpabp_lp_t Cpabp_lp_t Cpabp_lp_pi Lpabp_lp_pi Lpabr_lp_t Cpabr_lp_t Cpabr_lp_pi Lpabr_lp_pi bandera_calculo
sysprs=varargin{1};
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global f_paobp f_paobr palp_frec nump_max denp_max nump_min denp_min Wn_lp syspmax_lp_f syspmin_lp_f bandera_calculo 
...
global bandera_calculo
palp_frec=handles.palp_frec;
if palp_frec<=0 || ~isnumeric(palp_frec) || ~isreal(palp_frec) || isnan(palp_frec)
warndlg('Enter Numerical real denormalization frequency','Warning');
else
if bandera_calculo==0
