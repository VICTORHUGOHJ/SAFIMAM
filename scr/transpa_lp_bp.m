...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global f_paobp f_paobr nump_max denp_max nump_min denp_min Wc_bp Bw_bp syspmax_bp_f syspmin_bp_f pabp_frec pabp_bw 
global bandera_calculo_bp 
bandera_calculo_bp = 1;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global sysprs syspminrs npa denparcial numerador denminparcial numin nump_max denp_max nump_min denp_min valor_rs Lpabp_lp_t Cpabp_lp_t Cpabp_lp_pi Lpabp_lp_pi Lpabr_lp_t Cpabr_lp_t Cpabr_lp_pi Lpabr_lp_pi Wc_bp Bw_bp syspmax_bp_f syspmin_bp_f pabp_frec pabp_bw
global bandera_calculo_bp
pabp_frec=handles.pabp_frec;
pabp_bw=handles.pabp_bw;
if pabp_frec<=0 || ~isnumeric(pabp_frec) || ~isreal(pabp_frec) || pabp_bw<=0 || ~isnumeric(pabp_bw) || ~isreal(pabp_bw) || isnan(pabp_frec) || isnan(pabp_bw) 
